...
export class MasterMillsBulkController {
    ...
    async getTemplate(@Res({ passthrough: true }) res: Response): Promise<StreamableFile> {
    ...
    @ApiOperation({ summary: 'Upload an Excel file and preview parsed rows before import' })
    ...
    @HttpCode(202)
    @ApiOperation({ summary: 'Confirm import of a previously previewed upload session' })
    async confirmImport(@Body() dto: BulkUploadImportDto): Promise<{ message: string }> {
        await this.bulkService.confirmImport(dto.importId);
        return { message: 'Import started' };
    }

    @Get('bulk-upload/status/:importId')
    @ApiOperation({ summary: 'Get the current status of an ongoing or completed import' })
