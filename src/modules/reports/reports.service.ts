...
export class ReportsService {
  ...
  private formatChannelData(
    ...
    count?: number | null,
  ): { summary: string; channels: Record<number, string> } {
    ...
    // 2. Handle formatted strings like "Channel 1 (Secondary), Channel 2 (Primary)" or "Channel 1: Secondary"
    if (trimmed.includes(':') || trimmed.includes(',') || trimmed.includes('(')) {
      ...
      for (const part of parts) {
        ...
        // Matches "Channel 1 (Secondary)", "Channel 1: Secondary", "Ch 1: Primary", "1: Primary"
        const match = cleanPart.match(
          /(?:channel|ch)?\s*(\d+)\s*[:(]\s*([^)]+)\)?/i,
        );
        if (match) {
          ...
          const roleLabel =
            roleMapping[rawRole] || match[2].trim().replace(/_/g, ' ');
          if (chNum >= 1 && chNum <= 12) {
            channelMap[chNum] = roleLabel;
            summaryParts.push(`Channel ${chNum} (${roleLabel})`);
          } else {
          ...
          // Check for preset like "Split (6 Channels)"
          const presetMatch = cleanPart.match(
            /^([A-Za-z0-9_\s]+?)(?:\s*\(\s*(\d+)\s*channels?\s*\))?$/i,
          );
          if (presetMatch) {
    ...
    // 3. Single enum string or preset string (e.g. "PRIMARY" or "SPLIT")
  ...
  async exportInstallations(
    ...
    const dataRows = reports.map((r) => {
      const channelInfo = this.formatChannelData(
        r.running_channel_combination_value,
    ...
    if (formatType === 'excel') {
      ...
      // Style header row and apply full borders to all rows and columns
        ...
        row.eachCell({ includeEmpty: true }, (cell) => {
          if (rowNumber === 1) {
            ...
            cell.fill = {
              type: 'pattern',
              pattern: 'solid',
              fgColor: { argb: 'FFE56B00' }, // Mark Sorting Orange
            };
  ...
  private async enrichStoresWithMillAndRefNo(stores: any[]) {
    const frameNumbers = stores
      .map((s) => s.frame_number)
      .filter((fn): fn is string => Boolean(fn));

    if (frameNumbers.length === 0) return stores;
  ...
  private parseFullSerialMapFromRemarks(
    ...
    Array<{
      barcode: string;
      used: boolean;
      return_status?: 'Returned' | 'Not Returned' | 'Pending';
        ...
        if (bracketMatch && bracketMatch[1]) {
          ...
          const serials = rawSerials
            ...
            .map((s) => {
              ...
              const isNotReturned =
                /NOT_RETURNED|RET:Not Returned|Not Returned/i.test(s);
              ...
              return {
                ...
                return_status: used
                  ? isNotReturned
                    ? ('Not Returned' as const)
                    : ('Returned' as const)
                  : undefined,
                engineer_ack: used
                  ? engAckMatch
                    ? (engAckMatch[1] as 'Acknowledged' | 'Pending')
                    : 'Acknowledged'
                  : undefined,
                admin_ack: used
                  ? admAckMatch
                    ? (admAckMatch[1] as 'Acknowledged' | 'Pending')
                    : 'Pending'
                  : undefined,
              };
            })
