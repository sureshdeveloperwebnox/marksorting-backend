...
import { PreviewRow, PreviewResponse, ImportStatus } from './interfaces/bulk-upload.interface';
...
export class MasterMillsBulkService {
    ...
    async confirmImport(importId: string): Promise<void> {
        const rows = await this.redis.getJson<PreviewRow[]>(`bulk_upload:preview:${importId}`);
    ...
    async getImportStatus(importId: string): Promise<ImportStatus> {
        const status = await this.redis.getJson<ImportStatus>(`bulk_upload:status:${importId}`);
     ...
     */
    private async processImport(importId: string, rows: PreviewRow[]): Promise<void> {
