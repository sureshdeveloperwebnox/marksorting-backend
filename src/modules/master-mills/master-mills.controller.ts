...
export class MasterMillsController {
  ...
  findAll(
    ...
    if (dateFrom || dateTo) {
      ...
      if (dateFrom) {
        ...
        (where.invoice_date as any).gte = from;
      }
      if (dateTo) {
        const [ty, tm, td] = dateTo.split('-').map(Number);
        const to = new Date(ty, tm - 1, td, 23, 59, 59, 999);
        (where.invoice_date as any).lte = to;
      }
    }
  ...
  @ApiQuery({ name: 'context', required: false, type: String, description: 'Context of the report: service_report or installation_report' })
