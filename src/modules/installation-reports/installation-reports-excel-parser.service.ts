...
const HEADER_TO_FIELD_MAP: Record<string, RowField> = {
  ...
  'machine mfg date': 'machine_mfg_date',
  ...
  'warranty duration (months)': 'warranty_months',
          ...
          try {
            ...
            if (Array.isArray(parsed) && parsed.length > 0) {
              ...
              if (entries.length > 0) {
                ...
                if (!previewRow.running_channel_combination.trim()) {
                  previewRow.running_channel_combination = String(
                    entries.length,
                  );
                }
              }
            ...
            // Not JSON, continue to formatted string
          }

          // Try formatted string like "Channel 1: Secondary, Channel 2: Primary"
          if (
            !normalizedVal &&
            (rawVal.includes(':') || rawVal.includes(','))
          ) {
            const parts = rawVal.split(',');
            const entries: { channel: number; value: string }[] = [];
