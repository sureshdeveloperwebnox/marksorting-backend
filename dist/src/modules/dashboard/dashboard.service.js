...
let DashboardService = class DashboardService {
    ...
    async getDashboardData(startDate, endDate) {
        ...
        const calculateTrend = (current, previous) => {
            ...
            if (pct > 0)
                return { change: `+${pct.toFixed(1)}%`, trend: 'up' };
            if (pct < 0)
        ...
        const hasComparison = (arr) => arr.some((item) => Number(item.completed) > 0 || Number(item.pending) > 0);
        ...
        const getIntervalsForRange = (start, end) => {
            const result = [];
            const diffMs = end.getTime() - start.getTime();
            const stepMs = diffMs / 6;
            for (let i = 0; i <= 6; i++) {
        ...
        const getDaysForWeeklyTrend = (start, end) => {
            const result = [];
        ...
        const aggregateThisMonthTrend = (items, dateField, sumField) => {
            ...
            items.forEach((item) => {
                ...
                if (startDate && endDate) {
                    ...
                    let closestIdx = 0;
                    let minDiff = Math.abs(time - intervals[0].timestamp);
                    for (let i = 1; i < intervals.length; i++) {
                        const diff = Math.abs(time - intervals[i].timestamp);
                        if (diff < minDiff) {
                            minDiff = diff;
                            closestIdx = i;
                        }
                    ...
                    intervals[closestIdx].total += addVal;
                }
                else {
        ...
        const aggregateWeeklyTrend = (items, dateField, sumField) => {
            ...
            items.forEach((item) => {
                ...
                if (!dateVal)
                    return;
                const date = new Date(dateVal);
                const midnightTime = new Date(date.getFullYear(), date.getMonth(), date.getDate()).getTime();
        ...
        const finalContexts = {
            ...
            installations: {
                ...
                statusList: installationsCount > 0
        ...
        expensesPast12Months.forEach((e) => {
            if (e.visit_date) {
                const date = new Date(e.visit_date);
        ...
        const expenseRatio = past12MonthsExpenses.map((m, idx) => {
            ...
            const totalExpense = total12MonthExpense > 0
                ? total12MonthExpense
