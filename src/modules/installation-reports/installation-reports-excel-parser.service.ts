...
type RowField = keyof Omit<InstallationReportPreviewRow, 'errors' | 'isValid' | 'rowIndex'>;
...
export class InstallationReportsExcelParserService {
    ...
    async parseAndValidate(buffer: Buffer): Promise<InstallationReportPreviewRow[]> {
        const workbook = new ExcelJS.Workbook();
        ...
        worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
            ...
            for (const field of YES_NO_FIELDS) {
                const value = (previewRow[field] as string).trim().toLowerCase();
            ...
            // Integer fields
            const intFields: RowField[] = ['no_of_programs_set', 'no_of_filters_installed'];
