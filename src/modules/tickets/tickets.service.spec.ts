...
describe('TicketsService', () => {
    ...
    beforeEach(() => {
        ...
        prisma.mill.findFirst.mockResolvedValue({ id: 'mill-id', customer_id: 'customer-id' });
    ...
    it('rejects a mill that does not belong to the selected customer', async () => {
        prisma.mill.findFirst.mockResolvedValue({ id: 'mill-id', customer_id: 'different-customer-id' });
    ...
    it('validates the effective customer and mill on update', async () => {
        ...
        prisma.mill.findFirst.mockResolvedValue({ id: 'mill-id', customer_id: 'customer-id' });
        prisma.supportTicket.update.mockResolvedValue({ id: 'ticket-id', customer_id: 'customer-id' });

        await expect(service.update('ticket-id', { priority: 'HIGH' })).resolves.toEqual({
            id: 'ticket-id',
            customer_id: 'customer-id',
        });
