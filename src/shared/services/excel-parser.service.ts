...
const HEADER_TO_FIELD_MAP: Record<string, keyof Omit<PreviewRow, 'errors' | 'isValid' | 'rowIndex'>> = {
...
const DATE_FIELDS: Array<keyof PreviewRow> = ['invoice_date', 'installation_date', 'amc_starting_date', 'amc_closing_date'];
...
export class ExcelParserService {
    ...
    async parseAndValidate(buffer: Buffer): Promise<PreviewRow[]> {
        ...
        const headerRow = worksheet.getRow(1);
        const headerMap: Record<number, keyof Omit<PreviewRow, 'errors' | 'isValid' | 'rowIndex'>> = {};
        ...
        worksheet.eachRow({ includeEmpty: false }, (row, rowNumber) => {
            ...
            const rawData: Partial<Record<keyof Omit<PreviewRow, 'errors' | 'isValid' | 'rowIndex'>, string>> = {};
            ...
            for (const field of DATE_FIELDS) {
                ...
                if (value && value.trim() !== '') {
                    if (!this.isValidDate(value)) {
                        previewRow.errors[field as string] = `${field} must be a valid date`;
                    }
                }
            }
            ...
            for (const field of NUMERIC_FIELDS) {
                ...
                if (value && value.trim() !== '') {
                    if (!this.isNumeric(value)) {
                        previewRow.errors[field as string] = `${field} must be a numeric value`;
                    }
                }
            }
