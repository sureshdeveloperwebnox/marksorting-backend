...
const VALID_CHANNEL_VALUES = ['PRIMARY', 'SECONDARY', 'REJECTION_1', 'REJECTION_2', 'SPLIT'] as const;
...
export class InstallationReportsBulkService {
    ...
    async previewUpload(file: MulterFile): Promise<InstallationReportPreviewResponse> {
        if (!file) {
    ...
    async confirmImport(importId: string): Promise<void> {
        ...
        if (!rows) {
            throw new NotFoundException('Preview session expired. Please re-upload the file.');
        }

        void this.processImport(importId, rows);
    }

    async getImportStatus(importId: string): Promise<InstallationReportImportStatus> {
        const status = await this.redis.getJson<InstallationReportImportStatus>(
            `ir_bulk_upload:status:${importId}`,
        );

        if (!status) {
            throw new NotFoundException('Import status not found. The session may have expired.');
        }
    ...
    private async importSingleRow(row: InstallationReportPreviewRow): Promise<void> {
        await this.prisma.$transaction(async (tx) => {
            ...
            const channelVal = VALID_CHANNEL_VALUES.includes(channelValRaw as ChannelValue)
